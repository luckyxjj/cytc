...
<div class="container-fluid">
		...
		<div class="col-lg-9 table-bordered right-bg">
			...
			<table
				...
				<caption>
					<p class="title-info">工作环境</p>
					
					<a class="add-align btn btn-success " href="add_environment.jsp">新增</a>
				...
				<tbody id="member_img_info">
					<c:forEach items="${environmentList}" var="environment" varStatus="status">

						<tr>
							<th scope="row">${status.index + 1}</th>
							
							<td>${environment.imgUrl}</td>
							<td>${environment.createTime}</td>
							<td>${environment.updateTime}</td>

							<td><a href="environment/getEnvironmentById?id=${environment.id}"
								class="btn btn-primary"> 修改 </a></td>
							<td>
								<button id="delete" type="button"
									onclick="deleteEnvironment(${environment.id})" class="btn btn-primary">删除</button>
							</td>
						</tr>
