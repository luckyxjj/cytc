...
<div class="container-fluid">
		...
		<div class="col-lg-9 table-bordered right-bg">
			...
			<table
				...
				<caption>
					<p class="title-info">关于创艺图片管理</p>
					
					<a class="add-align btn btn-success " href="add_about.jsp">新增</a>
				...
				<tbody id="member_img_info">
					<c:forEach items="${aboutList}" var="about" varStatus="status">
						...
						<tr>
							...
							<td>
							<c:forEach items="${about.aboutImgList}" var="aboutImgList" varStatus="status">
							<a href="${aboutImgList.imgUrl}" target="_blank">
							<img src="${aboutImgList.imgUrl}"  width="80" height="60"></a>
							</c:forEach>
							</td>
							
							<td>${about.introduce}</td>
							<td>${about.culture}</td>
							<td>${about.createTime}</td>
							<td>${about.updateTime}</td>

							<td><a href="about/getAboutById?id=${about.id}"
								class="btn btn-primary"> 修改 </a></td>
							<td>
								<button id="delete" type="button"
									onclick="deleteAbout(${about.id})" class="btn btn-primary">删除</button>
							</td>
						</tr>
